...
int SensorAgent::readMemoryUsage()
{
    ...
    if (!process.waitForFinished(3000)) return -1;
    QString totalOut = QString::fromLocal8Bit(process.readAllStandardOutput()).trimmed();
    ...
    process.start("wmic", QStringList() << "os" << "get" << "freephysicalmemory" << "/value");
    if (!process.waitForFinished(3000)) return -1;
    QString freeOut = QString::fromLocal8Bit(process.readAllStandardOutput()).trimmed();
